...
class Path:
    ...
    @staticmethod
    def dir_path(map : Array2D[str], pos : Position, dir : Array2D.Direction) -> list[Position]:
    ...
    @staticmethod
    def group(map : Array2D[str], pos : Position, visited : set[Position] = set()) -> set[Position]:
    ...
    def truc(map : Array2D[str], pos : Position,) -> tuple[list[Position], int]:
        ...
        if not Path.is_directions_dict_valid({dir : directions[dir][0] != [] for dir in CARDINAUX}):
            raise ValueError("The group of blocks has an invalid movement setup")
