...
class Path:
    ...
    def is_directions_dict_valid(self, directions : dict[Array2D.Direction, bool])-> bool:
        ...
        return (directions.get(Array2D.Direction.N, False)+directions.get(Array2D.Direction.S, False))*(directions.get(Array2D.Direction.E, False)+directions.get(Array2D.Direction.W, False)) != 0

    def dir_path(self, map : Array2D[str], pos : Position, dir : Array2D.Direction) -> list[Position]:
    ...
    def group(self, map : Array2D[str], pos : Position, visited : set[Position] = set()) -> set[Position]:
        ...
        for dir in CARDINAUX:
            ...
            if map.at_position_with_direction(pos, dir) in {"=","-","X","E"} and newpos not in visited:
                visited.add(newpos)
                neighbour = neighbour | {newpos} | self.group(map, newpos, visited)
        return neighbour
    

        
                                                    #pour obtenir la liste des positions et l'index de départ
    def truc(self, map : Array2D[str], pos : Position,) -> tuple[list[Position], int]:
        ...
        group : set[Position] = self.group(map, pos)
        ...
        directions : dict[Array2D.Direction, tuple[list[Position], Position]] = {Array2D.Direction.N : ([], pos), Array2D.Direction.S : ([], pos), Array2D.Direction.E : ([], pos), Array2D.Direction.W : ([], pos)}
        for block in group :
            for dir in CARDINAUX:
        ...
        if not self.is_directions_dict_valid({dir : directions[dir][0] != [] for dir in CARDINAUX}):
            raise ValueError("The group of blocks has an invalid movement setup")
        ...
        paths : dict[Array2D.Direction, list[Position]] = {dir : [(directions[dir][0][i][0] + directions[dir][1][0] - pos[0], directions[dir][0][i][1] + directions[dir][1][1] - pos[1]) for i in range(len(directions[Array2D.Direction.N][0]))] for dir in CARDINAUX}
        ...
        strong_path : list[Position] = paths[Array2D.Direction.N] + paths[Array2D.Direction.E]
        ...
        weak_path : list[Position] = paths[Array2D.Direction.S] + paths[Array2D.Direction.W]
